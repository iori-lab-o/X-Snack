...
1.  **コンテキストのロード**: `docs/dev/[feature-name]/` 内の仕様書を読み込みます。
2.  **現状把握**: 修正対象のファイルや関連するコードを読み (`view_file`), 現在の実装スタイルを理解します。
3.  **スタイル確認**: リポジトリに `.eslintrc` や `.editorconfig`、`CONTRIBUTING.md` があるか確認し、あればそれに従います。規約がない場合は、言語標準のスタイル（例: TSならAirbnb/Google, PythonならPEP8）を採用します。
